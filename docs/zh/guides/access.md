...
每一行代表一个**渠道身份**，例如一个 Telegram 用户、Discord 成员或 Matrix 用户。每一行有两个互相独立的权限：
...
渠道聊天使用 ACL 引擎。v0.13 的 Access 页把它呈现为**访问模式**加可选的高级规则。
...
和旧 ACL 文档不同，v0.13 不再提供手动规则优先级或拖拽排序行为。理解时应按“当前默认模式 + 命中的反向规则”来判断。
